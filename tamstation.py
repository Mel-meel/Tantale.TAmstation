...
if __name__ == '__main__':
    ...
    class Handler(http.server.SimpleHTTPRequestHandler) :
        ...
        def do_GET(self) :
            ...
            self.end_headers()
            self.wfile.write(bytes("{temperature : " + str(temperature) + ", pression : " + str(pressure) + ", humidite : " + str(humidity) + "}", "utf-8"))
            #super(Handler, self).do_GET() #inherited do_GET serves dirs&files.

    #s = http.server.HTTPServer( ('', os.environ.get("PORT"), Handler )
    s = http.server.HTTPServer( ('', 8088), Handler )
    s.serve_forever()
